...
%                         is true.
%       'Hide'          : TRUE or FALSE specifying whether to show the text.
%                         Default is false.
%
...
%   MB: added options 4/5 for mouse wheel up/down.
%
...
addParamValue(p, 'ConnectPoints', true, @(x) validateattributes(x, ...
    {'logical'}, {'scalar'}));
addParamValue(p, 'Hide', false, @(x) validateattributes(x, ...
    {'logical'}, {'scalar'}));
...
% Try to prevent conflicts with mouse wheel MB
plotedit off
zoom off
pan off
rotate3d off
...
curWSWF = get(hFig, 'WindowScrollWheelFcn');
...
set(hFig, 'WindowScrollWheelFcn', @mouseWheelFcn);
...
%--------------------------------------------------------------------------
    function mouseWheelFcn(varargin)
...
%--------------------------------------------------------------------------
    function exitFcn()
        ...
        set(hFig, 'WindowScrollWheelFcn', curWSWF);
